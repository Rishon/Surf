...
public class ClientReferral
    ...
    public static ClientReferral deserialize(@Nonnull ByteBuf buf, int offset) {
        ...
        if ((nullBits & 2) != 0) {
            ...
            if ((long) (varPos1 + varIntLen) + (long) dataCount > (long) buf.readableBytes()) {
                throw ProtocolException.bufferTooSmall("Data", varPos1 + varIntLen + dataCount, buf.readableBytes());
            }
            obj.data = new byte[dataCount];
            for (int i = 0; i < dataCount; ++i) {
                obj.data[i] = buf.getByte(varPos1 + varIntLen + i);
            }
        }
        return obj;
    }
    ...
    public static ValidationResult validateStructure(@Nonnull ByteBuf buffer, int offset) {
        ...
        if ((nullBits & 2) != 0) {
            ...
            if ((pos += dataCount) > buffer.writerIndex()) {
    ...
    public int computeSize() {
        ...
        if (this.data != null) {
            size += VarInt.size(this.data.length) + this.data.length;
        }
        return size;
    }
    ...
    public boolean equals(Object obj) {
        ...
        if (!(obj instanceof ClientReferral other)) {
            return false;
        }
