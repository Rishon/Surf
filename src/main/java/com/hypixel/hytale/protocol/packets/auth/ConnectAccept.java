...
public class ConnectAccept
    ...
    public static ConnectAccept deserialize(@Nonnull ByteBuf buf, int offset) {
        ...
        if ((nullBits & 1) != 0) {
            ...
            if ((long) (pos + passwordChallengeVarLen) + (long) passwordChallengeCount > (long) buf.readableBytes()) {
                throw ProtocolException.bufferTooSmall("PasswordChallenge", pos + passwordChallengeVarLen + passwordChallengeCount, buf.readableBytes());
            }
            pos += passwordChallengeVarLen;
            obj.passwordChallenge = new byte[passwordChallengeCount];
            for (int i = 0; i < passwordChallengeCount; ++i) {
                obj.passwordChallenge[i] = buf.getByte(pos + i);
            }
            pos += passwordChallengeCount;
        }
        return obj;
    }
    ...
    public static int computeBytesConsumed(@Nonnull ByteBuf buf, int offset) {
        ...
        if ((nullBits & 1) != 0) {
            ...
            pos += VarInt.length(buf, pos) + arrLen;
        }
        return pos - offset;
    }
    ...
    public static ValidationResult validateStructure(@Nonnull ByteBuf buffer, int offset) {
        ...
        if ((nullBits & 1) != 0) {
            ...
            if ((pos += passwordChallengeCount) > buffer.writerIndex()) {
    ...
    public int computeSize() {
        int size = 1;
        if (this.passwordChallenge != null) {
            size += VarInt.size(this.passwordChallenge.length) + this.passwordChallenge.length;
        }
        return size;
    }
    ...
    public boolean equals(Object obj) {
        ...
        if (!(obj instanceof ConnectAccept other)) {
            return false;
        }
