...
public class PasswordResponse
    ...
    public static PasswordResponse deserialize(@Nonnull ByteBuf buf, int offset) {
        ...
        if ((nullBits & 1) != 0) {
            ...
            if ((long) (pos + hashVarLen) + (long) hashCount > (long) buf.readableBytes()) {
                throw ProtocolException.bufferTooSmall("Hash", pos + hashVarLen + hashCount, buf.readableBytes());
            }
            pos += hashVarLen;
            obj.hash = new byte[hashCount];
            for (int i = 0; i < hashCount; ++i) {
                obj.hash[i] = buf.getByte(pos + i);
            }
            pos += hashCount;
        }
        return obj;
    ...
    public static ValidationResult validateStructure(@Nonnull ByteBuf buffer, int offset) {
        ...
        if ((nullBits & 1) != 0) {
            ...
            if ((pos += hashCount) > buffer.writerIndex()) {
    ...
    public int computeSize() {
        int size = 1;
        if (this.hash != null) {
            size += VarInt.size(this.hash.length) + this.hash.length;
        }
        return size;
    }
    ...
    public boolean equals(Object obj) {
        ...
        if (!(obj instanceof PasswordResponse other)) {
            return false;
        }
