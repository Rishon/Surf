...
public class AssetPart
    ...
    public static AssetPart deserialize(@Nonnull ByteBuf buf, int offset) {
        ...
        if ((nullBits & 1) != 0) {
            ...
            if ((long) (pos + partVarLen) + (long) partCount > (long) buf.readableBytes()) {
                throw ProtocolException.bufferTooSmall("Part", pos + partVarLen + partCount, buf.readableBytes());
            }
            pos += partVarLen;
            obj.part = new byte[partCount];
            for (int i = 0; i < partCount; ++i) {
                obj.part[i] = buf.getByte(pos + i);
            }
            pos += partCount;
        }
        return obj;
    ...
    public static ValidationResult validateStructure(@Nonnull ByteBuf buffer, int offset) {
        ...
        if ((nullBits & 1) != 0) {
            ...
            if ((pos += partCount) > buffer.writerIndex()) {
    ...
    public int computeSize() {
        int size = 1;
        if (this.part != null) {
            size += VarInt.size(this.part.length) + this.part.length;
        }
        return size;
    }
    ...
    public boolean equals(Object obj) {
        ...
        if (!(obj instanceof AssetPart other)) {
            return false;
        }
