...
public class PasswordRejected
    ...
    public static PasswordRejected deserialize(@Nonnull ByteBuf buf, int offset) {
        ...
        if ((nullBits & 1) != 0) {
            ...
            if ((long) (pos + newChallengeVarLen) + (long) newChallengeCount > (long) buf.readableBytes()) {
                throw ProtocolException.bufferTooSmall("NewChallenge", pos + newChallengeVarLen + newChallengeCount, buf.readableBytes());
            }
            pos += newChallengeVarLen;
            obj.newChallenge = new byte[newChallengeCount];
            for (int i = 0; i < newChallengeCount; ++i) {
                obj.newChallenge[i] = buf.getByte(pos + i);
            }
            pos += newChallengeCount;
        }
        return obj;
    }
    ...
    public static ValidationResult validateStructure(@Nonnull ByteBuf buffer, int offset) {
        ...
        if ((nullBits & 1) != 0) {
            ...
            if ((pos += newChallengeCount) > buffer.writerIndex()) {
    ...
    public int computeSize() {
        int size = 5;
        if (this.newChallenge != null) {
            size += VarInt.size(this.newChallenge.length) + this.newChallenge.length;
        }
        return size;
    }
    ...
    public boolean equals(Object obj) {
        ...
        if (!(obj instanceof PasswordRejected other)) {
            return false;
        }
