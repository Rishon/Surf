...
public class Connect
    ...
    public static Connect deserialize(@Nonnull ByteBuf buf, int offset) {
        ...
        if ((nullBits & 2) != 0) {
            ...
            if ((long) (varPos3 + varIntLen) + (long) referralDataCount > (long) buf.readableBytes()) {
                throw ProtocolException.bufferTooSmall("ReferralData", varPos3 + varIntLen + referralDataCount, buf.readableBytes());
            }
            obj.referralData = new byte[referralDataCount];
            for (int i = 0; i < referralDataCount; ++i) {
                obj.referralData[i] = buf.getByte(varPos3 + varIntLen + i);
            }
        }
        if ((nullBits & 4) != 0) {
    ...
    public static int computeBytesConsumed(@Nonnull ByteBuf buf, int offset) {
        ...
        if ((nullBits & 2) != 0) {
            ...
            if ((pos3 += VarInt.length(buf, pos3) + arrLen) - offset > maxEnd) {
                maxEnd = pos3 - offset;
            }
        }
        if ((nullBits & 4) != 0) {
    ...
    public static ValidationResult validateStructure(@Nonnull ByteBuf buffer, int offset) {
        ...
        if ((nullBits & 2) != 0) {
            ...
            pos += VarInt.length(buffer, pos);
            if ((pos += referralDataCount) > buffer.writerIndex()) {
    ...
    public int computeSize() {
        ...
        if (this.referralData != null) {
            size += VarInt.size(this.referralData.length) + this.referralData.length;
        }
        if (this.referralSource != null) {
    ...
    public boolean equals(Object obj) {
        ...
        if (!(obj instanceof Connect other)) {
            return false;
        }
