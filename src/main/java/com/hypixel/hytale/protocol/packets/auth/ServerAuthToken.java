...
public class ServerAuthToken
    ...
    public static ServerAuthToken deserialize(@Nonnull ByteBuf buf, int offset) {
        ...
        if ((nullBits & 2) != 0) {
            ...
            if ((long) (varPos1 + varIntLen) + (long) passwordChallengeCount > (long) buf.readableBytes()) {
                throw ProtocolException.bufferTooSmall("PasswordChallenge", varPos1 + varIntLen + passwordChallengeCount, buf.readableBytes());
            }
            obj.passwordChallenge = new byte[passwordChallengeCount];
            for (int i = 0; i < passwordChallengeCount; ++i) {
                obj.passwordChallenge[i] = buf.getByte(varPos1 + varIntLen + i);
            }
        }
        return obj;
    }
    ...
    public static int computeBytesConsumed(@Nonnull ByteBuf buf, int offset) {
        ...
        if ((nullBits & 2) != 0) {
            ...
            if ((pos1 += VarInt.length(buf, pos1) + arrLen) - offset > maxEnd) {
                maxEnd = pos1 - offset;
            }
        }
        return maxEnd;
    ...
    public boolean equals(Object obj) {
        ...
        if (!(obj instanceof ServerAuthToken other)) {
            return false;
        }
